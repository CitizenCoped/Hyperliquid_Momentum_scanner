...
import { getAdminToken, isAdminAuthError, setAdminToken } from "@/lib/admin-token";
  ...
  const onSubmit = (data: SettingsFormValues) => {
    ...
    updateSettings.mutate({ data }, {
      ...
      onError: (err) => {
        toast({
          title: "Error",
          description: isAdminAuthError(err)
            ? "Admin API token is missing, invalid, or not configured on the server."
            ...
            <div className="bg-card border border-border rounded-lg p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Required for saving settings and test notifications"
                  className="font-mono bg-background"
                  autoComplete="off"
                />
                <p className="text-xs text-muted-foreground">
                  Stored only in this browser session and sent as a bearer token for admin actions.
                  It must match SCANNER_ADMIN_TOKEN on the API server.
                </p>
              </div>
            </div>
