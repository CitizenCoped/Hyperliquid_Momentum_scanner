...
describe("parsePerpSnapshots", () => {
  ...
  it("rejects empty universes so transient upstream payloads cannot clear scanner state", () => {
    assert.throws(
  ...
  it("rejects misaligned contexts so partial upstream payloads cannot drop assets", () => {
    assert.throws(
      () =>
        parsePerpSnapshots([
