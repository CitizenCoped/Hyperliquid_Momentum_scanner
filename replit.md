...
- `artifacts/api-server/src/routes/{scanner,alerts,settings}.ts` — REST routes wired in `routes/index.ts`. Settings PUT validates payload with Zod and enforces `watch <= active <= A+` invariant server-side. Mutating routes require `Authorization: Bearer <SCANNER_ADMIN_TOKEN>`.
...
- `SCANNER_ADMIN_TOKEN` — bearer token required for saving settings, dismissing alerts, and sending test notifications.
