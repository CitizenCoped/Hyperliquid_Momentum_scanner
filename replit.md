...
- `artifacts/api-server/src/routes/{scanner,alerts,settings}.ts` — REST routes wired in `routes/index.ts`. Settings PUT validates payload with Zod and enforces `watch <= active <= A+` invariant server-side. Mutating admin routes require a bearer token matching `SCANNER_ADMIN_TOKEN` (or `API_ADMIN_TOKEN`).
...
- `SCANNER_ADMIN_TOKEN` (or `API_ADMIN_TOKEN`) — bearer token for settings updates, alert dismissal, and test pushes.
