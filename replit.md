...
- `artifacts/api-server/src/routes/{scanner,alerts,settings}.ts` — REST routes wired in `routes/index.ts`. Settings PUT validates payload with Zod and enforces `watch <= active <= A+` invariant server-side. Admin mutations require bearer auth.
...
- `SCANNER_ADMIN_TOKEN` or `ADMIN_API_TOKEN` (falls back to `SESSION_SECRET`) — required for settings changes, alert dismissal, and Pushover tests.
